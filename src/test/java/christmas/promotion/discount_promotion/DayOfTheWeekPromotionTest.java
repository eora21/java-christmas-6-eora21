...
class DayOfTheWeekPromotionTest {
    ...
    void orderOutOfPromotionDayOfWeek(LocalDate localDate, String dayOfTheWeek) {
        Map<Menu, Discount> menuDiscount = dayOfTheWeekPromotion.calculateMenuDiscount(localDate, orders);
        assertThat(menuDiscount).isEmpty();
    }
    ...
    void orderInPromotionDayOfWeek(LocalDate localDate, String dayOfTheWeek) {
        Map<Menu, Discount> menuDiscount = dayOfTheWeekPromotion.calculateMenuDiscount(localDate, orders);
        assertThat(menuDiscount).containsKeys(Menu.TAPAS, Menu.MUSHROOM_SOUP);
    }
    ...
    void calculateDiscountAmountNotIncludeDiscountMenuGroup() {
        ...
        Map<Menu, Discount> menuDiscount = dayOfTheWeekPromotion.calculateMenuDiscount(
                LocalDate.parse("2023-12-16"), ordersNotIncludeAppetizer);

        assertThat(menuDiscount).isEmpty();
    }

    @Test
    @DisplayName("할인 금액은 일치하는 에피타이저 총 주문량 * 할인 가격(2,023)이다.")
    void calculateDiscountAmount() {
        Map<Menu, Discount> menuDiscount = dayOfTheWeekPromotion.calculateMenuDiscount(
                LocalDate.parse("2023-12-16"), orders);
        assertThat(menuDiscount.get(Menu.TAPAS).getDiscountAmount()).isEqualTo(new Amount(2 * 2_023));
        assertThat(menuDiscount.get(Menu.MUSHROOM_SOUP).getDiscountAmount()).isEqualTo(new Amount(3 * 2_023));
    }
