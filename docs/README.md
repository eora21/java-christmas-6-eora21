...
만약 1,000원짜리 음료 메뉴가 생겼다면, 2,023원을 깎아주었을 때 0원이 되어야 합니다. 값이 음수가 되어버리면 관계 없는 메뉴들이 할인받게 되거나, 혹은 오히려 음식값을 역으로 줘야 하는 것처럼 되어버릴 수도 있기 때문입니다.
### 총주문 금액에서 할인
...
그래도 한 주제에 대해 깊게 고민하고, 스스로 결정하고 진행하며 많은 것들을 배우고 깨달을 수 있어 정말 좋은 기회였다고 생각합니다.
꼭 본 코스에도 참여하여 많은 분들과 더 깊은 생각을 나누고 모두 같이 성장할 수 있길 바랍니다.